...
def compute_critic_loss(
    ...
    if q_target is None:
        q_target = q_values
    # Use the current Q network for choosing the action for the next state
    next_actions = q_values[1].argmax(dim=-1, keepdim=True)
    # Use the target Q network (Q') for calculating the Q-value of that action in the next state
    max_q = q_target[1].gather(dim=-1, index=next_actions).detach().squeeze(dim=-1)
